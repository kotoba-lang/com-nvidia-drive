(ns nvidia_drive.main
  "Kotodama WASM entrypoint for the NVIDIA Drive clean-room actor (L4) — Clojure port.
  ...
  facts (`nvidia_drive.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nvidia_drive")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nvidia_drive.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nvidia_drive-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nvidia_drive-compat/ready)
